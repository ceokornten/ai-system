...
│   └── encrypt.js           # Helper to encrypt secrets for .env
├── utils/
...
1. Copy the example env file and fill in your credentials:
   ...
   ```
   Secrets can be stored encrypted by using the `*_ENC` variables. Run
   the helper script to encrypt a value (requires `CONFIG_SECRET`):
   ```bash
   node scripts/encrypt.js "my-api-key"
   ```
   Use the printed string as the value of `OPENAI_API_KEY_ENC`,
   `LINE_CHANNEL_SECRET_ENC`, or `LINE_ACCESS_TOKEN_ENC` in your `.env`.
   You can also override the OpenAI model by setting `OPENAI_MODEL`.
